...
int getDifferentFiles(char *_original_path, char *_change_path) {
	...
	while ((_dirp = readdir(_change_dp)) != NULL) {
		...
		if (_dirp->d_type == DT_DIR) {
			...
			snprintf(_change_subdirentory_path, sizeof(_change_subdirentory_path), "%s/%s", _change_path, _dirp->d_name);
		...
		//if dirp is file
			...
			if (_original_file != NULL) {
				...
				//set file command
				snprintf(_original_file_command, sizeof(_original_file_command), "md5sum %s/%s", _original_path, _dirp->d_name);
				snprintf(_change_file_command, sizeof(_change_file_command), "md5sum %s/%s", _change_path, _dirp->d_name);
